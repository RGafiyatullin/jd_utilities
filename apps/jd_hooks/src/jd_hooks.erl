...
-export_type([ret/1]).
-include("jd_hooks.hrl").

-type ret( Acc_t ) :: {continue, Acc_t} | {stop, Acc_t} | stop.

-spec add_handler( HookID :: term(), Handler :: hook_handler(), Priority :: integer() ) -> ok.
add_handler( HookID, Handler, Priority ) -> jd_hooks_hook_mgr:add_handler(jd_hooks_hook_mgr:get( HookID ), Handler, Priority ).
...
run( HookID, AccIn, Args ) -> jd_hooks_hook_runner_beam:run( HookID, AccIn, Args ).

run_pipeline( HookIDs, AccIn, Args ) -> jd_hooks_hook_runner_ets:run_multiple( HookIDs, AccIn, Args ).
