...
%%% %%% %%%
-spec get( term() ) -> pid().
get( HookID ) -> {ok, HookMgr} = jd_hooks_hook_mgr_sup:get_hook_mgr( HookID ), HookMgr.

-spec add_handler( pid(), {mfa, module(), function(), [term()]}, integer() ) -> ok.
...
init({ HookID }) -> {ok, #s{ hook_id = HookID, handlers = [ #handler{ func = F, priority = P } || {F, P} <- jd_hooks_ets:handlers_get( HookID ) ] }}.
...
%%% Internal %%%
%%% %%%%%%%% %%%
-spec do_add_handler( {mfa, module(), function(), [term()]}, integer(), #s{} ) -> {reply, ok, #s{}}.
do_add_handler( {mfa, M, F, A}, Priority, State = #s{ hook_id = HookID, handlers = Handlers } ) ->
	...
	jd_hooks_ets:handlers_set( HookID, [ {Func, Prio} || #handler{ func = Func, priority = Prio } <- NewHandlers] ),
	jd_hooks_compiler:notify_hook_handlers_updated( HookID ),
